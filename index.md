...
[`print()`](https://rdrr.io/r/base/print.html) is console-only and never
writes quarto syntax into a rendered document, even when called from
inside a code chunk; use
[`knitr::knit_print()`](https://rdrr.io/pkg/knitr/man/knit_print.html)
in a chunk with `#| results: asis` to actually emit the syntax. This
matters in practice because the console summary produced by
[`print()`](https://rdrr.io/r/base/print.html) is written via `cli`,
which routes through [`message()`](https://rdrr.io/r/base/message.html)
when not attached to an interactive terminal (as during `quarto render`)
— and some formats (e.g. `revealjs`) suppress `message`/`warning` chunk
output by default, so a stray
[`print()`](https://rdrr.io/r/base/print.html) left in a chunk can
silently produce nothing under those formats even though it evaluated
correctly.
